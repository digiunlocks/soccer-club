...
In consideration of being allowed to participate as a volunteer in Seattle Leopards FC programs, related events, and activities, the undersigned acknowledges, appreciates, and agrees to the following:

1. I understand and accept the risks of injury, disability, death, and property loss that may result from volunteering or participating in club activities, whether arising from my own actions, the actions of others, or the condition of the facilities or equipment used.
2. I agree to comply with all club rules, policies, and codes of conduct, and to promote a safe, respectful, and positive environment for all participants, coaches, officials, and spectators.
3. I certify that I am physically fit and capable of fulfilling my volunteer duties, and that I will immediately notify club officials of any condition that may affect my ability to volunteer safely.
...
5. I understand that as a volunteer, I am expected to act with integrity, fairness, and in the best interest of the club at all times.
...
By agreeing below, I acknowledge that I have read, understood, and voluntarily accept the terms of this waiver and release of liability as a condition of my participation as a volunteer with Seattle Leopards FC.
`;
...
export default function VolunteerApplication() {
  ...
  const validate = () => {
    ...
    if (!form.interests) newErrors.interests = "Required";
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validation).length === 0) {
      try {
        const res = await api.post(API_ENDPOINTS.APPLICATIONS.BASE, { type: "volunteer", info: form });
        setSubmitted(true);
        setForm({
          name: "",
          dob: "",
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded shadow mt-8">
      ...
      <h2 className="text-2xl font-bold mb-4 text-green-900">Volunteer Application</h2>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div>
          <label className="font-semibold">Areas of Interest / Skills <span className="text-red-600">*</span></label>
          <textarea name="interests" value={form.interests} onChange={handleChange} className="border border-green-300 rounded px-3 py-2 w-full" rows={2} required />
          {errors.interests && <div className="text-xs text-red-600">{errors.interests}</div>}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
        ...
        <div className="bg-green-50 border border-green-200 rounded p-3 text-sm">
          <label className="flex items-start gap-2">
            ...
            />
            <span>
              I confirm that all information provided is accurate and I am applying to join Seattle Leopards FC as a volunteer. <span className="text-red-600">*</span>
            </span>
          </label>
